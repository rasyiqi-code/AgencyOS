...
import { getAffiliateAssetsFn } from '@/src/server/marketing'

export const Route = createFileRoute('/affiliate/resources')({
  beforeLoad: async () => {
  ...
  loader: async () => {
    ...
    if (!dashboardData) throw redirect({ href: '/handler/sign-in' })
    if (!dashboardData.profile) throw redirect({ to: '/affiliate/join' })
    ...
    const assets = await getAffiliateAssetsFn()

    return {
      referralCode: dashboardData.profile.referralCode,
      agencyName,
      assets,
    }
  },
...
function MarketingKitPage() {
  const { referralCode, agencyName, assets } = Route.useLoaderData()
  ...
  const processContent = (content: string) => {
    const refLink = `${origin}?ref=${referralCode}`
  ...
  const getAssets = (type: string) => assets.filter((a: MarketingAsset) => a.type === type)
      ...
      <Tabs defaultValue="copy" className="w-full">
        ...
        <TabsContent value="copy" className="mt-6 space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            {getAssets('copy').map((asset: MarketingAsset) => {
              const finalContent = processContent(asset.content || '')
              return (
                <Card key={asset.id} className="bg-zinc-900 border-zinc-800">
                  <CardHeader className="pb-2">
                    <div className="flex justify-between items-start">
                      <Badge variant="outline" className="text-xs bg-black border-zinc-800 text-zinc-400">
            ...
            {getAssets('copy').length === 0 && (
              <div className="text-zinc-500 col-span-full text-center py-10">
                No copywriting assets available.
              </div>
            )}
          </div>
        </TabsContent>
        ...
        <TabsContent value="banner" className="mt-6 space-y-4">
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {getAssets('banner').map((asset: MarketingAsset) => (
              <Card key={asset.id} className="bg-zinc-900 border-zinc-800 overflow-hidden">
                <div className="aspect-video bg-black relative group">
                  ...
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                    <Button
                      size="sm"
                      variant="secondary"
                      onClick={() => downloadImage(asset.imageUrl || '', `banner-${asset.id}.png`)}
                    >
                ...
                <CardContent className="p-4">
                  <div className="flex justify-between items-center mb-2">
                    <Badge variant="outline" className="text-[10px] bg-black border-zinc-800 text-zinc-400">
                      {asset.category || 'Social'}
                    </Badge>
                  </div>
                  <h3 className="font-medium text-white line-clamp-1" title={asset.title}>
                    {asset.title}
                  </h3>
                  ...
                  <div className="mt-4 pt-4 border-t border-zinc-800/50">
                    ...
                    <div className="flex bg-black rounded border border-zinc-800 p-1">
                      <code className="text-[10px] text-zinc-400 font-mono flex-1 overflow-hidden whitespace-nowrap px-2 py-1">
                        {`<a href="${origin}?ref=${referralCode}"><img src="${asset.imageUrl}" alt="${asset.title}" /></a>`}
                      </code>
                      <button
                        onClick={() =>
                          copyToClipboard(
        ...
        <TabsContent value="banner_widget" className="mt-6 space-y-4">
          <div className="grid gap-6">
            {getAssets('banner_widget').map((asset: MarketingAsset) => (
              <Card key={asset.id} className="bg-zinc-900 border-zinc-800 border-l-4 border-l-purple-600">
                ...
                <CardContent className="space-y-4">
                  <div className="aspect-[3/1] bg-black/50 rounded-lg overflow-hidden border border-zinc-800 relative">
                    {asset.imageUrl && (
                      <img
                        src={asset.imageUrl}
                        alt={asset.title}
                  ...
                  <p className="text-sm text-zinc-400">
                    This banner will automatically update if the admin changes the official artwork. Copy the code
                    below to embed this specific banner on your site.
                  </p>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          copyToClipboard(
                            `<script src="${origin}/widgets/banner-ad.js" data-host="${origin}" data-id="${asset.id}" data-ref="${referralCode}"></script>`,
                          )
                        }
                      >
                        <Copy className="w-4 h-4 mr-2" /> Copy
            ...
            {getAssets('banner_widget').length === 0 && (
              <div className="text-zinc-500 py-10 text-center">No managed banner ads available yet.</div>
            )}
          </div>
        </TabsContent>
        ...
        <TabsContent value="widget" className="mt-6 space-y-4">
          <Card className="bg-zinc-900 border-zinc-800 border-l-4 border-l-blue-600">
            ...
            <CardContent className="space-y-4">
              <p className="text-sm text-zinc-400">
                This is a smart floating badge that appears at the bottom-right of any website. It&apos;s designed
                to be non-intrusive yet effective for conversion.
              </p>
                ...
                <div className="flex items-center gap-3">
                  <div className="bg-zinc-950 border border-zinc-800 rounded-full px-4 py-2 flex items-center gap-3 shadow-lg">
                    <span className="text-xs text-white">
                      Powered by <strong>{agencyName}</strong>
                    </span>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      copyToClipboard(
                        `<script src="${origin}/widgets/affiliate-badge.js" data-ref="${referralCode}" data-name="${agencyName}"></script>`,
                      )
                    }
                  >
                ...
                <div className="bg-black p-4 rounded-lg border border-zinc-800 font-mono text-xs text-blue-400 overflow-x-auto whitespace-pre">
                  {`<script 
  src="${origin}/widgets/affiliate-badge.js" 
...
></script>`}
          ...
          <div className="relative py-4">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-zinc-950 px-2 text-zinc-500 font-medium">Custom Admin Widgets</span>
            </div>
          </div>

          <div className="grid gap-6">
            {getAssets('widget').map((asset: MarketingAsset) => {
              const finalContent = processContent(asset.content || '')
              return (
                <Card key={asset.id} className="bg-zinc-900 border-zinc-800">
                  <CardHeader>
                      ...
                      <div className="bg-zinc-950 p-6 rounded-lg border border-zinc-800/50 flex items-center justify-center min-h-[120px]">
                        <div className="text-center space-y-2">
                          ...
                          <p className="text-xs text-zinc-600 max-w-[200px]">
                            This custom widget will render dynamically based on the provided script.
                          </p>
                        </div>
                      </div>
                    </div>
            ...
            {getAssets('widget').length === 0 && (
              <div className="text-zinc-500 py-10 text-center">No widgets available.</div>
            )}
          </div>
        </TabsContent>
      </Tabs>
