...
const loader = async () => {
  const settings = await getSystemSettings({
    data: ['CONTACT_EMAIL', 'CONTACT_PHONE', 'CONTACT_ADDRESS', 'CONTACT_HOURS'],
  })
  const pageSeo = await getPageSeoFn({ data: '/contact' })
...
export const Route = createFileRoute('/contact')({
  ...
  head: ({ loaderData }) => {
    ...
    const description = pageSeo?.description || 'Get in touch with us for your software development needs.'
...
function ContactPage() {
  ...
  const info = {
    email: settings?.find((s: SystemSetting) => s.key === 'CONTACT_EMAIL')?.value || null,
    phone: settings?.find((s: SystemSetting) => s.key === 'CONTACT_PHONE')?.value || null,
    address: settings?.find((s: SystemSetting) => s.key === 'CONTACT_ADDRESS')?.value || null,
    hours: settings?.find((s: SystemSetting) => s.key === 'CONTACT_HOURS')?.value || null,
  }
  ...
  return (
    <div className="min-h-screen bg-black">
      <SiteHeader />
      <div className="container mx-auto px-4 py-24 sm:py-32 selection:bg-blue-500/30">
        {/* Back Link */}
        <div className="mb-8">
          <Link
            to="/"
            className="inline-flex items-center text-sm text-zinc-500 hover:text-white transition-colors gap-1"
          >
