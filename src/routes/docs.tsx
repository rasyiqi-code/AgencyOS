...
const loader = async () => {
  const settings = await getSystemSettings({ data: ['COMPANY_NAME'] })
  const pageSeo = await getPageSeoFn({ data: '/docs' })
...
export const Route = createFileRoute('/docs')({
  ...
  head: ({ loaderData }) => {
    ...
    const title = pageSeo?.title || 'Integration Documentation'
    const description = pageSeo?.description || 'Choose your product integration path.'
...
function DocumentationPage() {
  ...
  if (!type || (type !== 'saas' && type !== 'license')) {
    return (
      <div className="min-h-screen bg-black">
        <SiteHeader />
        <div className="min-h-screen bg-[#09090b] text-zinc-400 font-sans selection:bg-brand-yellow/30 flex flex-col items-center justify-center py-20 px-6 pt-28">
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-zinc-500 leading-relaxed">
                    {isId 
                      ...
                      : 'Verify user access in real-time via our robust API.'}
                  </p>
                </div>
  ...
  return (
    <div className="min-h-screen bg-black">
      <SiteHeader />
      <div className="min-h-screen bg-[#09090b] text-zinc-400 font-sans selection:bg-brand-yellow/30 pb-24 pt-24">
        {/* Guide Header */}
