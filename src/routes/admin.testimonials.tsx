...
import { getAllTestimonialsFn, toggleTestimonialStatusFn, deleteTestimonialFn } from '@/src/server/admin'

export const Route = createFileRoute('/admin/testimonials')({
  loader: async () => {
    return getAllTestimonialsFn()
  },
...
function AdminTestimonialsRoute() {
    ...
    const toggleStatusMutation = useMutation({
        mutationFn: async ({ id, currentStatus }: { id: string; currentStatus: boolean }) => {
            const result = await toggleTestimonialStatusFn({ data: { id, isActive: !currentStatus } })
            ...
            <div className="space-y-6">
                ...
                <div className="block lg:hidden space-y-3">
                    {loading ? (
                        ...
                        testimonials.map((t) => (
                            <div key={t.id} className="p-3 md:p-4 rounded-2xl border border-white/5 bg-zinc-900/30 space-y-2.5 text-left">
                                ...
                                <div className="text-[10px] text-zinc-400 leading-relaxed italic border-l-2 border-yellow-500/30 pl-3">
                                    &quot;{t.content}&quot;
                                </div>
                                        ...
                                        <Button
                                            ...
                                            className={cn(
                                                ...
                                                t.isActive
                                                    ...
                                                    : "border-yellow-500/20 bg-yellow-500/10 text-yellow-500 hover:bg-yellow-500 hover:text-black"
                                            )}
                ...
                <div className="hidden lg:block relative group overflow-hidden rounded-3xl border border-white/5 bg-zinc-900/20 backdrop-blur-xl">
                    <Table>
                        <TableHeader className="bg-white/5">
                            <TableRow className="border-white/5 hover:bg-transparent">
                                <TableHead className="text-zinc-500 font-bold uppercase text-[10px] tracking-widest pl-8 text-left">User</TableHead>
                                <TableHead className="text-zinc-500 font-bold uppercase text-[10px] tracking-widest text-left">Feedback Content</TableHead>
                                <TableHead className="text-zinc-500 font-bold uppercase text-[10px] tracking-widest text-left">Status</TableHead>
                                <TableHead className="text-zinc-500 font-bold uppercase text-[10px] tracking-widest text-left">Submission Date</TableHead>
                            ...
                            {loading ? (
                                ...
                                testimonials.map((t) => (
                                    <TableRow key={t.id} className="border-white/5 hover:bg-white/[0.02] transition-colors">
                                        ...
                                        <TableCell className="max-w-[400px] text-left">
                                            <div className="text-[11px] text-zinc-400 leading-relaxed line-clamp-2 italic border-l border-yellow-500/30 pl-3 py-1">
                                                &quot;{t.content}&quot;
                                            </div>
                                        </TableCell>
                    ...
                    {/* Efek Kaca Berkilau */}
                    <div className="absolute -top-24 -left-24 w-48 h-48 bg-yellow-500/10 blur-[100px] pointer-events-none opacity-50" />
                    <div className="absolute -bottom-24 -right-24 w-48 h-48 bg-yellow-500/5 blur-[100px] pointer-events-none opacity-50" />
                </div>
