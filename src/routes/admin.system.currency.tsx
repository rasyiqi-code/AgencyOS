...
import { getCurrencyConfigFn, saveCurrencyConfigFn, forceUpdateCurrencyRatesFn } from '@/src/server/admin'

export const Route = createFileRoute('/admin/system/currency')({
  loader: async () => {
    return getCurrencyConfigFn()
  },
  component: CurrencySettingsRoute,
})
...
function CurrencySettingsRoute() {
    const initialData = Route.useLoaderData()
    const queryClient = useQueryClient()

    // Mengambil data konfigurasi mata uang menggunakan React Query
    const { data: currencyData, isLoading, error } = useQuery<CurrencyData>({
        queryKey: ["currencyConfig"],
        queryFn: () => getCurrencyConfigFn(),
...
function CurrencySettingsForm({ initialConfig, initialRates, queryClient }: CurrencySettingsFormProps) {
    ...
    const saveMutation = useMutation({
        mutationFn: async () => {
            await saveCurrencyConfigFn({ data: { apiKey, intervalHours: Number(intervalHours) } })
        },
        onSuccess: () => {
    ...
    const forceUpdateMutation = useMutation({
        mutationFn: async () => {
            return await forceUpdateCurrencyRatesFn()
        },
        onSuccess: () => {
    ...
    return (
        <div className="space-y-8 p-4 md:p-8 max-w-4xl mx-auto w-full">
            <div>
