...
import { getAdminProjectDetailFn } from '@/src/server/pm'
import { SafeImage } from "@/components/ui/safe-image"
import { Terminal, MessageSquare, Activity, Calendar, Clock } from "lucide-react"
import { StatusSelector } from "@/components/admin/pm/detail/status-selector"
import { TechnicalAssetManager } from "@/components/admin/pm/detail/asset-manager"
import { PreviewUploader } from "@/components/admin/pm/detail/preview-uploader"
import { FileManager } from "@/components/admin/pm/detail/file-manager"
import { DailyLogFeed } from "@/components/admin/pm/detail/daily-log-feed"
...
export const Route = createFileRoute('/admin/pm/projects/$id')({
  loader: async ({ params: { id } }) => {
    return getAdminProjectDetailFn({ data: id })
  },
...
function AdminProjectDetailRoute() {
  ...
  // Sinkronisasi data detail proyek dengan React Query
  const { data } = useQuery({
    queryKey: ['admin-project-detail', id],
    queryFn: () => getAdminProjectDetailFn({ data: id }),
    initialData,
  })
  ...
  const project = data.project as unknown as ExtendedProject
  const team = data.team as unknown as ExtendedSquadProfile[]

  return (
    <div className="w-full py-4 pb-12 text-left">
              ...
              <div className="p-4 rounded-xl border border-white/5 bg-zinc-900/40">
                <div className="flex flex-col md:flex-row gap-4">
                  ...
                  <div className="flex-1 space-y-3">
                    ...
                    {/* Display Estimate Screens/APIs if available */}
