...
import { getClientMissionDetailFn } from '@/src/server/client-dashboard'
...
import { FileManager } from '@/components/admin/pm/detail/file-manager'
...
import { type ProjectFile, type ExtendedSquadProfile } from '@/lib/shared/types'

export const Route = createFileRoute('/dashboard/missions/$id')({
  loader: async ({ params }) => {
    return await getClientMissionDetailFn({ data: params.id })
  },
...
function ProjectDetailPage() {
  ...
  if (!result || !result.success || !result.project) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] gap-4 text-left">
        <h1 className="text-2xl font-bold text-red-500">Gagal Memuat</h1>
        <p className="text-zinc-400">Detail misi tidak ditemukan atau Anda tidak memiliki akses.</p>
        <Link to="/dashboard/missions">
          <Button variant="outline">Kembali ke Log Misi</Button>
        </Link>
      </div>
    )
  }

  const { project, assignedProfile } = result
  ...
  }[project.status as 'queue' | 'dev' | 'review' | 'done'] || project.status

  return (
    <div className="pb-6 min-h-screen text-left w-full">
        ...
        {project.repoUrl && (
          <a href={project.repoUrl} target="_blank" rel="noreferrer">
            <Button
              variant="outline"
              size="icon"
              className="h-7 w-7 bg-zinc-900 border-white/10 text-zinc-400 hover:text-white"
            >
      ...
      <div className="grid gap-4 grid-cols-1 lg:grid-cols-3">
          ...
          <div className="p-4 rounded-xl border border-white/5 bg-zinc-900/40">
            ...
            {project.service?.description && (
              <div
                ...
                dangerouslySetInnerHTML={{ __html: sanitizeHtml(project.service.description) }}
              />
            )}
                ...
                <div className="flex flex-col md:flex-row gap-4">
                  ...
                  <div className="flex-1 space-y-3">
                    ...
                    {!!project.service?.features && (
                      <ServiceFeaturesList features={(project.service.features as string[]) || []} />
                    )}
                      ...
                      <div className="grid grid-cols-3 gap-2 mt-2">
                        <div className="p-2 bg-black/30 rounded border border-white/5">
                          ...
                          <span className="text-sm font-mono text-zinc-300">
                            {(project.estimate.screens as unknown[]).length} Pages
                          </span>
                        </div>
                        <div className="p-2 bg-black/30 rounded border border-white/5">
                          ...
                          <span className="text-sm font-mono text-zinc-300">
                            {(project.estimate.apis as unknown[]).length} Endpoints
                          </span>
                        </div>
                        <div className="p-2 bg-black/30 rounded border border-white/5">
                          <span className="text-[10px] text-zinc-500 uppercase block">
                          ...
                          <span className="text-sm font-mono text-zinc-300">
                            {project.estimate.totalHours} Hours
                          </span>
                        </div>
                      </div>
                    )}
                    ...
                    {project.spec && (
                      <div className="mt-4 pt-4 border-t border-white/5">
                        <h4 className="text-[9px] font-semibold text-zinc-500 uppercase mb-2">
                          Technical Specs
                        </h4>
        ...
        <div className="space-y-4 sticky top-6 self-start">
              ...
              <div className="space-y-3">
                <div className="space-y-1.5">
                  <div className="flex justify-between items-center text-[10px]">
                    ...
                    <Badge
                      variant="outline"
                      className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 text-[9px] px-1.5 py-0"
                    >
                      Online
                    </Badge>
                  </div>
          ...
          <AssignedTeamCard
            ...
            assignedProfile={assignedProfile as unknown as ExtendedSquadProfile}
