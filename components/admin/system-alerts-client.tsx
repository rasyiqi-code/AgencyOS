...
export function SystemAlertsClient({ aiConfigured, gatewayConfigured }: Props) {
  // Gunakan state bawaan untuk menyimpan status tutup
  const [isDismissed, setIsDismissed] = useState(true) // Default true untuk mencegah flash sebelum mount

  useEffect(() => {
    // Muat preferensi dari sessionStorage agar notifikasi tidak muncul kembali saat navigasi halaman di sesi yang sama
    const dismissed = sessionStorage.getItem('system_alerts_dismissed') === 'true'
    setIsDismissed(dismissed)
  }, [])

  const handleDismiss = () => {
    setIsDismissed(true)
    sessionStorage.setItem('system_alerts_dismissed', 'true')
  }

  if (isDismissed || (aiConfigured && gatewayConfigured)) {
    return null
  }

  return (
    <div className="relative w-full bg-amber-950/10 border border-amber-500/20 rounded-2xl p-5 mb-6 backdrop-blur-md shadow-xl shadow-amber-950/5 transition-all duration-300">
      {/* Tombol Tutup Peringatan */}
      <button
        onClick={handleDismiss}
        className="absolute top-4 right-4 p-1.5 rounded-lg text-amber-500/50 hover:text-amber-500 hover:bg-amber-500/10 transition-colors cursor-pointer"
