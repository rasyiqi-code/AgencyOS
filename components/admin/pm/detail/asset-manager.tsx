...
export function TechnicalAssetManager({ project }: TechnicalAssetManagerProps) {
    ...
    const handleSave = async () => {
        setIsLoading(true);
        try {
            const repoUrl = form.repoOwner && form.repoName ? `https://github.com/${form.repoOwner}/${form.repoName}` : null;
            ...
            if (!result.success) throw new Error("Gagal memperbarui aset");

            toast.success("Aset teknis berhasil diperbarui");
            setIsEditing(false);
            router.invalidate();
        } catch (error) {
            console.error(error);
            toast.error("Gagal memperbarui aset teknis");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="p-4 rounded-xl border border-blue-500/20 bg-blue-500/5 space-y-4 animate-in fade-in slide-in-from-top-2 text-left">
            ...
            <div className="space-y-3">
                ...
                <Button
                    onClick={handleSave}
                    disabled={isLoading}
                    className="w-full bg-blue-600 hover:bg-blue-500 text-white h-8 text-xs gap-2 font-bold uppercase"
                >
